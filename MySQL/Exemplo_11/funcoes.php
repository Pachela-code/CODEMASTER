...
{
    return selectSQLUnico("SELECT nome, telemovel FROM clientes WHERE id=$id");
}

function atualizarUltimoAcesso($login)
{
    $data = date("H:i:s - d/m/Y");
    $id = selectSQLUnico("SELECT id FROM administradores WHERE login='$login'")["id"];
